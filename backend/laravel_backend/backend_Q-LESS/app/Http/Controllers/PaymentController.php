...
class PaymentController extends Controller
{
    ...
    private function orderEmailHtml(Order $order): string
    {
        ...
        foreach ($items as $item) {
            ...
            $name = htmlspecialchars((string) ($item['nombre'] ?? 'Producto Q-LESS'), ENT_QUOTES, 'UTF-8');
            $quantity = max(1, (int) ($item['cantidad'] ?? 1));
            $subtotal = number_format((float) ($item['subtotal'] ?? 0), 0, ',', '.');
            $discountLabel = htmlspecialchars((string) ($item['discount_label'] ?? ''), ENT_QUOTES, 'UTF-8');
            $discountAmount = (float) ($item['descuento'] ?? 0);
            $discountHtml = '';
            ...
            if ($discountLabel !== '' || $discountAmount > 0) {
                $discountText = $discountLabel !== ''
                    ? $discountLabel
                    : 'Descuento aplicado';
                $discountHtml = '<div style="margin-top:6px;color:#2f8f35;font-size:13px;font-weight:700;">' . $discountText . '</div>';
            }

            $itemsHtml .= <<<HTML
              <tr>
                <td style="padding:12px;border-bottom:1px solid #e3efda;color:#1f3d24;font-weight:700;">{$name}{$discountHtml}</td>
