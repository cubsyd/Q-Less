...
class PaymentController extends Controller
{
    public function createPreference(Request $request)
    {
        ...
        $notificationUrl = trim((string) config('services.mercadopago.notification_url'));
    ...
    public function createOrder(Request $request)
    {
        ...
        CartReservation::where('user_id', $user->id)->delete();

        Mail::to($user->email)
            ->send(new DeliveryCodeMail($orderNumber));
    ...
    {
        $baseUrl = rtrim(
            (string) config('services.mercadopago.frontend_url'),
            '/'
        );
